    ...
    if (this->state)
    {
        if (this->hard_fault_counter < FAULT_COUNTER_LIMIT)
        {
            ...
            while (this->fault_flag == true)
            {
                ...
                this->channel_on_impl(); // turn off the channel and turn it on to reset the hardware latch
