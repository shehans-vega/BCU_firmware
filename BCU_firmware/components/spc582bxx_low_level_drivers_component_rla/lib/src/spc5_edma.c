...
static uint8_t dma1_g0[16] = {SPC5_EDMA1_GROUP0_PRIORITIES};
#if (SPC5_EDMA_NCHANNELS > 16U) || defined(__DOXYGEN__)
static uint8_t dma1_g1[16] = {SPC5_EDMA1_GROUP1_PRIORITIES};
#endif
#if (SPC5_EDMA_NCHANNELS > 32U) || defined(__DOXYGEN__)
static uint8_t dma1_g2[16] = {SPC5_EDMA1_GROUP2_PRIORITIES};
static uint8_t dma1_g3[16] = {SPC5_EDMA1_GROUP3_PRIORITIES};
#endif
#if (SPC5_EDMA_NCHANNELS > 64U) || defined(__DOXYGEN__)
static uint8_t dma0_g0[16] = {SPC5_EDMA0_GROUP0_PRIORITIES};
#endif
#if (SPC5_EDMA_NCHANNELS > 80U) || defined(__DOXYGEN__)
static uint8_t dma0_g1[16] = {SPC5_EDMA0_GROUP1_PRIORITIES};
#endif
 ...
 * @notapi
 */
static void edmaSetMuxSlot(edma_channel_config_t *ccfg) {

  uint8_t mux_cfg = (uint8_t)((uint8_t)EDMA_CHANNEL_ENABLE | (uint8_t)(ccfg->dma_channel & 0x40U) | (uint8_t)(ccfg->dma_periph & 0x3FU));
#if (SPC5_EDMA_NUM_OF_MUX > 1U)
  uint32_t source = ccfg->dma_channel & 0x3FU;

  if(ccfg->dma_mux == 0U) {
    /* Programming the MUX0.*/
    SPC5_DMAMUX0.CHCONFIG[source].R = mux_cfg;
  } else if (ccfg->dma_mux == 1U) {
    source = source - SPC5_EDMA_MUX0_CH_NUM;
    /* Programming the MUX1.*/
    SPC5_DMAMUX1.CHCONFIG[source].R = mux_cfg;
    ...
    SPC5_DMAMUX2.CHCONFIG[source].R = mux_cfg;
#endif
    ...
    SPC5_DMAMUX3.CHCONFIG[source].R = mux_cfg;
#endif
    ...
    SPC5_DMAMUX4.CHCONFIG[source].R = mux_cfg;
#endif
    ...
    SPC5_DMAMUX5.CHCONFIG[source].R = mux_cfg;
#endif
  } else {
...
#else
  /* Programming the MUX.*/
  SPC5_DMAMUX0.CHCONFIG[ccfg->dma_channel & 0x3FU].R = mux_cfg;
 ...
 * @special
 */
edma_channel_t edmaChannelAllocate(edma_channel_config_t *ccfg) {

  uint8_t dma_channel = (uint8_t)(ccfg->dma_channel & 0x3FU);

  /* If the channel is already taken then an error is returned.*/
  if (channels[dma_channel] != NULL) {
    return EDMA_ERROR;
  }
  ...
  /*lint -e9032 -e9033*/
  INTC_PSR(SPC5_EDMA_CH0_NUMBER + dma_channel) = ccfg->dma_irq_prio;
  /*lint +e9032 +e9033*/
 ...
 * @notapi
 */
static void edmaResetMuxSlot(edma_channel_config_t *ccfg) {
#if (SPC5_EDMA_NUM_OF_MUX > 1U)
  uint32_t source = ccfg->dma_channel & 0x3FU;
 ...
 * @special
 */
void edmaChannelRelease(edma_channel_config_t *ccfg) {

  uint8_t dma_channel = (uint8_t)(ccfg->dma_channel & 0x3FU);

  /* Enforcing a stop.*/
  edmaChannelStop(dma_channel);

  edmaResetMuxSlot(ccfg);

  /* Clearing ISR sources for the channel.*/
#if (SPC5_EDMA_NCHANNELS <= 64U)
  SPC5_EDMA1.CINT.R = dma_channel;
  SPC5_EDMA1.CEEI.R = dma_channel;
  SPC5_EDMA1.CERR.R = dma_channel;
#else
  if (ccfg->dma_channel < 64U) {
    SPC5_EDMA1.CINT.R = dma_channel;
    SPC5_EDMA1.CEEI.R = dma_channel;
    ...
    SPC5_EDMA0.CINT.R = dma_channel - 64U;
    SPC5_EDMA0.CEEI.R = dma_channel - 64U;
    SPC5_EDMA0.CERR.R = dma_channel - 64U;
  }
#endif /* SPC5_EDMA_NCHANNELS <= 64U */

  /* The channels is flagged as available.*/
  channels[dma_channel] = NULL;
}
