...
static LinConfig default_config = {
...
static void ld_rxi(LinDriver *ldp, uint8_t *rx_buf, size_t len) {
  ...
  while (len > 0U) {
    ...
    for (i = 0; ((i < len) && (i < 4U)); i++) {
      *rx_buf++ = tData.b[max_index - i];    /* Endianess to take in account */
    }
 ...
 * @api
 */
void lin_lld_start(LinDriver *ldp, LinConfig *config) {
